...
import { Alert, App as AntdApp, Button, Card, Col, Descriptions, Empty, Form, Input, Modal, Popconfirm, Row, Select, Space, Statistic, Table, Tag, Typography } from 'antd';
...
export function StoragePanel() {
  ...
  const [ingestVisibility, setIngestVisibility] = useState<'private' | 'public'>('private');
  ...
  const createEndpoint = async () => {
    try { const values = await form.validateFields(); setIngestVisibility((values.visibility === 'public' ? 'public' : 'private')); await api.createStorageEndpoint(values); form.resetFields(); message.success('存储源已登记，正在自动扫描并接入影像'); await load(); }
  ...
  const ingest = async () => {
    ...
    try { await api.ingestStorageObjects(selectedEndpoint.id, { object_ids: selectedObjectIds, visibility: ingestVisibility }); message.success('引用登记任务已创建'); setSelectedObjectIds([]); }
    catch (error) { message.error(normalizeError(error)); }
  };
  const activeObjects = useMemo(() => objects.filter((item) => !item.missing_confirmed), [objects]);
  return <div className="data-page">
    <div className="page-header"><div><Typography.Title level={2}>存储管理</Typography.Title><Typography.Text type="secondary">登记目录后自动扫描并入库存量影像</Typography.Text></div><Button icon={<ReloadOutlined />} onClick={() => void load()}>刷新</Button></div>
